        ...
        {[
          {
            ...
            sub: interviewJobs > 0 ? `${interviewJobs} in interview` : "No interviews yet",
            icon: Briefcase,
            href: "/dashboard/jobs",
          ...
          },
          {
            ...
            value: String(tasksDueSoon > 0 ? tasksDueSoon : allTasks.filter((t) => t.status !== "DONE").length),
            sub: overdueTasks > 0
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                {s.badge && (
                  <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${s.badgeStyle}`}>
                    {s.badge}
                  </span>
                )}
              </div>
              <div className="text-4xl font-bold text-white mb-1 tabular-nums">{s.value}</div>
              <div className="text-slate-200 text-sm font-semibold">{s.label}</div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          ...
          <div className="bg-slate-800/50 border border-slate-700/60 rounded-2xl p-5">
            ...
            <div className="space-y-2">
              {[
                { href: "/dashboard/jobs",  label: "Add job application", icon: Briefcase,   hover: "hover:bg-violet-500/15  hover:border-violet-400/40  border-violet-500/20",  color: "text-violet-300"  },
                { href: "/dashboard/tasks", label: "Create a task",       icon: Target,      hover: "hover:bg-amber-500/15   hover:border-amber-400/40   border-amber-500/20",   color: "text-amber-300"   },
                { href: "/dashboard/notes", label: "Write a note",        icon: FileText,    hover: "hover:bg-emerald-500/15 hover:border-emerald-400/40 border-emerald-500/20", color: "text-emerald-300" },
              ].map((a) => {
                const Icon = a.icon;
                return (
                  <Link
          ...
          <div className="bg-slate-800/50 border border-slate-700/60 rounded-2xl p-5">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { label: "Applied",   count: appliedJobs,   color: "text-indigo-300",  bg: "bg-indigo-500/10  border-indigo-500/25"  },
                { label: "Interview", count: interviewJobs, color: "text-violet-300",  bg: "bg-violet-500/10  border-violet-500/25"  },
                { label: "Offer",     count: offerJobs,     color: "text-emerald-300", bg: "bg-emerald-500/10 border-emerald-500/25" },
              ].map((s) => (
                <div key={s.label} className={`text-center py-2.5 px-2 rounded-xl border ${s.bg}`}>
            ...
            <div className="mt-3 pt-3 border-t border-slate-700/50 grid grid-cols-2 gap-2">
              {[
                { label: "Active Tasks",    count: allTasks.filter((t) => t.status !== "DONE").length, color: "text-amber-300",   bg: "bg-amber-500/10  border-amber-500/20"  },
                { label: "Done This Week",  count: allTasks.filter((t) => t.status === "DONE" && +new Date(t.updatedAt) > +now - 7 * 86_400_000).length, color: "text-emerald-300", bg: "bg-emerald-500/10 border-emerald-500/20" },
              ].map((s) => (
                <div key={s.label} className={`text-center py-2 px-2 rounded-xl border ${s.bg}`}>
                  <p className={`text-xl font-bold tabular-nums ${s.color}`}>{s.count}</p>
                  <p className="text-slate-500 text-[10px] font-medium mt-0.5">{s.label}</p>
                </div>
              ))}
